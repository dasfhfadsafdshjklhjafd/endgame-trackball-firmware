...
struct zip_pointer_2s_mixer_config {
    ...
    const bool single_sensor_transform_defined;
...
struct zip_pointer_2s_mixer_data {
    ...
    bool single_sensor_mode;
    bool single_sensor_use_sensor2;
    ...
    bool single_sensor_transform_enabled;
...
static void apply_single_sensor_transform(struct zip_pointer_2s_mixer_data *data, float *x, float *y);
...
static void apply_single_sensor_transform(struct zip_pointer_2s_mixer_data *data, float *x, float *y) {
    if (!data->single_sensor_transform_enabled) {
        return;
    }

    const float in_x = *x;
    const float in_y = *y;
    *x = data->single_sensor_transform[0][0] * in_x +
         data->single_sensor_transform[0][1] * in_y;
    *y = data->single_sensor_transform[1][0] * in_x +
         data->single_sensor_transform[1][1] * in_y;
}
...
static int sy_handle_event(const struct device *dev, struct input_event *event, const uint32_t p1,
    ...
    struct zip_pointer_2s_mixer_data *data = dev->data;
    const int64_t now = k_uptime_get();
    const bool single_sensor_mode = data->single_sensor_mode;
    const bool single_sensor_uses_sensor2 = data->single_sensor_use_sensor2;
    ...
    if (p1 & INPUT_MIXER_SENSOR1) {
        if (!(single_sensor_mode && single_sensor_uses_sensor2)) {
#if IS_ENABLED(CONFIG_POINTER_2S_MIXER_ENSURE_SYNC)
...
static int data_init(const struct device *dev) {
    ...
    float surface_p1[3] = {0}, surface_p2[3] = {0};
    data->single_sensor_mode = config->single_sensor_mode;
    data->single_sensor_use_sensor2 = data->single_sensor_mode && config->single_sensor_primary == 2;
    const bool need_sensor1_data = !data->single_sensor_mode || !data->single_sensor_use_sensor2;
    const bool need_sensor2_data = !data->single_sensor_mode || data->single_sensor_use_sensor2;
    float rotation_matrix1[3][3];
    float rotation_matrix2[3][3];
    bool have_surface1 = false;
    bool have_surface2 = false;
    ...
    data->single_sensor_transform_enabled =
        data->single_sensor_mode && config->single_sensor_transform_defined;
    for (size_t row = 0; row < 2; row++) {
        for (size_t col = 0; col < 2; col++) {
            data->single_sensor_transform[row][col] =
                (float)config->single_sensor_transform[row * 2 + col] / 1000.0f;
        }
    }

    if (data->single_sensor_transform_enabled) {
        LOG_INF("Single-sensor transform enabled (a=%d b=%d c=%d d=%d)",
                (int)config->single_sensor_transform[0],
                (int)config->single_sensor_transform[1],
                (int)config->single_sensor_transform[2],
                (int)config->single_sensor_transform[3]);
    }

    if (need_sensor1_data) {
        ...
        have_surface1 = true;
        calculate_rotation_matrix(surface_p1[0], surface_p1[1], surface_p1[2], 0, 0, -radius, rotation_matrix1);
    } else {
        for (size_t row = 0; row < 3; row++) {
            for (size_t col = 0; col < 3; col++) {
                rotation_matrix1[row][col] = (row == col) ? 1.0f : 0.0f;
            }
        }
    }

    if (need_sensor2_data) {
        if (!line_sphere_intersection(radius,
        ...
        have_surface2 = true;
        calculate_rotation_matrix(surface_p2[0], surface_p2[1], surface_p2[2], 0, 0, -radius, rotation_matrix2);
    } else {
        for (size_t row = 0; row < 3; row++) {
            for (size_t col = 0; col < 3; col++) {
                rotation_matrix2[row][col] = (row == col) ? 1.0f : 0.0f;
            }
        }
    }

    if (have_surface1 && have_surface2 &&
        surface_p1[0] == surface_p2[0] &&
        surface_p1[1] == surface_p2[1] &&
        surface_p1[2] == surface_p2[2]) {
        LOG_ERR("Unexpected: both trackpoints have same coordinates!");
        return 0;
    }

    if (data->single_sensor_mode) {
        if (data->single_sensor_use_sensor2) {
            memcpy(data->rotation_matrix1, rotation_matrix2, sizeof(data->rotation_matrix1));
        } else {
            memcpy(data->rotation_matrix1, rotation_matrix1, sizeof(data->rotation_matrix1));
        }

        for (size_t row = 0; row < 3; row++) {
            for (size_t col = 0; col < 3; col++) {
                data->rotation_matrix2[row][col] = (row == col) ? 1.0f : 0.0f;
            }
        }
    } else {
        memcpy(data->rotation_matrix1, rotation_matrix1, sizeof(data->rotation_matrix1));
        memcpy(data->rotation_matrix2, rotation_matrix2, sizeof(data->rotation_matrix2));
    }
    ...
    if (have_surface1) {
        ...
        LOG_DBG("  > Surface trackpoint 1: using identity (unused)");
    }

    if (have_surface2) {
        LOG_DBG("  > Surface trackpoint 2 ≈ (%d, %d, %d)", (int) surface_p2[0], (int) surface_p2[1], (int) surface_p2[2]);
    } else {
        LOG_DBG("  > Surface trackpoint 2: using identity (unused)");
    }
...
static struct zip_pointer_2s_mixer_config config = {
    ...
    .single_sensor_mode = DT_INST_PROP_OR(0, single_sensor_mode, false),
    .single_sensor_primary = DT_INST_PROP_OR(0, single_sensor_primary, 1),
#if DT_INST_NODE_HAS_PROP(0, single_sensor_transform)
    .single_sensor_transform = DT_INST_PROP(0, single_sensor_transform),
    .single_sensor_transform_defined = true,
#else
    .single_sensor_transform = {1000, 0, 0, 1000},
    .single_sensor_transform_defined = false,
#endif
